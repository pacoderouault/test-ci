...
public class GeneCommentActionTableCell extends TableCell<ReportGeneCommentary, Void> {

    private final static Logger logger = LogManager.getLogger(GeneCommentActionTableCell.class);
    private final HBox box = new HBox();

    public GeneCommentActionTableCell() {
    ...
    private void showComment() {
        ReportGeneCommentary commentary = getTableRow().getItem();
        if (commentary != null) {
            Set<String> targetGenes = new HashSet<>();
            targetGenes.add(commentary.getGeneName());
    ...
    private void editComment() {
        ...
        if (user.isPermitted(PermissionsEnum.EDIT_REPORT_COMMENT)) {
            ...
            if (commentary != null) {
                ...
                dialog.getButton(ButtonType.OK).setOnAction(event -> {
                    try {
                        DAOController.get().getReportGeneCommentaryDAO().editReportGeneCommentary(
                                commentary.getId(),
        ...
        if (user.isPermitted(PermissionsEnum.EDIT_REPORT_COMMENT)) {
            ...
            if (commentary != null) {
                ...
                dialog.getButton(ButtonType.YES).setOnAction(event -> {
                    try {
                        DAOController.get().getReportGeneCommentaryDAO().deleteReportGeneCommentary(commentary.getId());
