...
public class CaddPopupFilter extends PredictionPopupFilter2{

    public CaddPopupFilter(FilterTableColumn<Annotation, VariantPrediction> tableColumn) {
        super(tableColumn);
    }
    ...
    protected void updatePredicate(Operators op, Number value) {
        if (value == null) {
            ...
            getTableColumn().setPredicate(a -> {
                switch (op) {
                    case EQUALS:
                        return a.getTranscriptConsequence().getCaddPhredPred().getScore().doubleValue() == value.doubleValue();
                    case NOT_EQUALS:
                        return a.getTranscriptConsequence().getCaddPhredPred().getScore().doubleValue() != value.doubleValue();
                    case GREATER_THAN:
                        return a.getTranscriptConsequence().getCaddPhredPred().getScore().doubleValue() > value.doubleValue();
