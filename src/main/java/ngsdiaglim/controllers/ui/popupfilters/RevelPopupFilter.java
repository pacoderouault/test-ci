...
public class RevelPopupFilter extends PredictionPopupFilter2{

    public RevelPopupFilter(FilterTableColumn<Annotation, VariantPrediction> tableColumn) {
        super(tableColumn);
    }
    ...
    protected void updatePredicate(Operators op, Number value) {
        if (value == null) {
            ...
            getTableColumn().setPredicate(a -> {
                switch (op) {
                    case EQUALS:
                        return a.getTranscriptConsequence().getRevelPred().getScore().doubleValue() == value.doubleValue();
                    case NOT_EQUALS:
                        return a.getTranscriptConsequence().getRevelPred().getScore().doubleValue() != value.doubleValue();
                    case GREATER_THAN:
                        return a.getTranscriptConsequence().getRevelPred().getScore().doubleValue() > value.doubleValue();
