...
public class PolyphenHdivPopupFilter extends PredictionPopupFilter2{

    public PolyphenHdivPopupFilter(FilterTableColumn<Annotation, VariantPrediction> tableColumn) {
        super(tableColumn);
    }
    ...
    protected void updatePredicate(Operators op, Number value) {
        if (value == null) {
            ...
            getTableColumn().setPredicate(a -> {
                switch (op) {
                    case EQUALS:
                        return a.getTranscriptConsequence().getPolyphen2HdivPred().getScore().doubleValue() == value.doubleValue();
                    case NOT_EQUALS:
                        return a.getTranscriptConsequence().getPolyphen2HdivPred().getScore().doubleValue() != value.doubleValue();
                    case GREATER_THAN:
                        return a.getTranscriptConsequence().getPolyphen2HdivPred().getScore().doubleValue() > value.doubleValue();
