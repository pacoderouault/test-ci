 ...
 */
public class Zoomer2 extends ChartPlugin {
    private static final Logger LOGGER = LogManager.getLogger(Zoomer2.class);
    ...
    private final ObservableDeque<Map<Axis, Zoomer2.ZoomState>> zoomStacks = new ObservableDeque<>(new ArrayDeque<>());
     ...
     */
    public Zoomer2(final AxisMode zoomMode) {
        this(zoomMode, false);
    }
     ...
     */
    public Zoomer2(final AxisMode zoomMode, final boolean animated) {
        super();
        ...
        chartProperty().addListener((change, o, n) -> {
            ...
            if (n != null) {
                ...
                final Zoomer2.ZoomRangeSlider slider = new Zoomer2.ZoomRangeSlider(n);
                if (isSliderVisible()) {
     ...
     * @param animated initial value of {@link #animatedProperty() animated} property
     */
    public Zoomer2(final boolean animated) {
    ...
    public void clear(final Axis axis) {
        for (Map<Axis, Zoomer2.ZoomState> stackStage : zoomStacks) {
    ...
    public boolean zoomOrigin() {
        ...
        final Map<Axis, Zoomer2.ZoomState> zoomWindows = zoomStacks.peekLast();
     ...
     */
    public ObservableDeque<Map<Axis, Zoomer2.ZoomState>> zoomStackDeque() {
        return zoomStacks;
    }
    ...
    private Map<Axis, Zoomer2.ZoomState> getZoomDataWindows() {
        ConcurrentHashMap<Axis, Zoomer2.ZoomState> axisStateMap = new ConcurrentHashMap<>();
        if (getChart() == null) {
            return axisStateMap;
        }
        ...
        for (Axis axis : getChart().getAxes()) {
            ...
            switch (getAxisMode()) {
                case X:
                    if (axis.getSide().isHorizontal()) {
                        axisStateMap.put(axis,
                                new Zoomer2.ZoomState(dataMin, dataMax, axis.isAutoRanging(), axis.isAutoGrowRanging()));
                    }
                    break;
                case Y:
                    if (axis.getSide().isVertical()) {
                ...
                case XY:
                default:
                    axisStateMap.put(axis, new Zoomer2.ZoomState(dataMin, dataMax, axis.isAutoRanging(), axis.isAutoGrowRanging()));
                    break;
            }
        }
    ...
    private boolean isOmitZoomInternal(final Axis axis) {
        final boolean propertyState = Zoomer2.isOmitZoom(axis);
    ...
    private void performZoom(Entry<Axis, Zoomer2.ZoomState> zoomStateEntry, final boolean isZoomIn) {
        Zoomer2.ZoomState zoomState = zoomStateEntry.getValue();
    ...
    private void performZoom(Map<Axis, Zoomer2.ZoomState> zoomWindows, final boolean isZoomIn) {
        for (final Entry<Axis, Zoomer2.ZoomState> entry : zoomWindows.entrySet()) {
    ...
    private void pushCurrentZoomWindows() {
        ...
        for (Axis axis : getChart().getAxes()) {
            switch (getAxisMode()) {
                case X:
                    if (axis.getSide().isHorizontal()) {
                        axisStateMap.put(axis, new Zoomer2.ZoomState(axis.getMin(), axis.getMax(), axis.isAutoRanging(),
                ...
                default:
                    axisStateMap.put(axis,
                            new Zoomer2.ZoomState(axis.getMin(), axis.getMax(), axis.isAutoRanging(), axis.isAutoGrowRanging())); // NOPMD
                    // necessary
                    // in-loop
                    // instantiation
                    break;
    ...
    private boolean zoomOut() {
        clearZoomStackIfAxisAutoRangingIsEnabled();
        final Map<Axis, Zoomer2.ZoomState> zoomWindows = zoomStacks.pollFirst();
        if (zoomWindows == null || zoomWindows.isEmpty()) {
            return zoomOrigin();
        }
        performZoom(zoomWindows, false);
