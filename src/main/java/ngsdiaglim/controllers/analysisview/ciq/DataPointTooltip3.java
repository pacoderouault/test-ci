 ...
 */
public class DataPointTooltip3 extends AbstractDataFormattingPlugin {
    /**
     ...
     */
    public DataPointTooltip3(final double pickingDistance) {
        this();
    ...
    protected void updateLabel(final MouseEvent event, final Bounds plotAreaBounds, final DataPoint dataPoint) {
        ...
        if (spaceRight >= width + LABEL_X_OFFSET) { // place to right if enough space
            xLocation = mouseX + DataPointTooltip3.LABEL_X_OFFSET;
        } else if (spaceLeft >= width + LABEL_X_OFFSET) { // place left if enough space
            xLocation = mouseX - DataPointTooltip3.LABEL_X_OFFSET - width;
        ...
        if (height < spaceBottom) {
            yLocation = mouseY + DataPointTooltip3.LABEL_Y_OFFSET;
        } else if (height < spaceTop) {
            yLocation = mouseY - DataPointTooltip3.LABEL_Y_OFFSET - height;
