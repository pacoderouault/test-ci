...
public class ScrollBarUtil {
    ...
    public static void testscroll(TableView<?> control) {
        ScrollBar scrollBar = getScrollbarComponent(control, Orientation.VERTICAL);
        scrollBar.setBlockIncrement(20);
    }

    private static ScrollBar getScrollbarComponent(TableView<?> control, Orientation orientation) {
    ...
    public static void smoothScrollingTableView(TableView<?> tableView, double speed) {
        smoothScrollingTableView(tableView, speed, Orientation.VERTICAL, bounds -> bounds.getHeight());
    }

    public static void smoothHScrollingTableView(TableView<?> tableView, double speed) {
        smoothScrollingTableView(tableView, speed, Orientation.HORIZONTAL, bounds -> bounds.getHeight());
    }

    private  static void smoothScrollingTableView(TableView<?> tableView, double speed, Orientation orientation, Function<Bounds, Double> sizeFunc) {
        ScrollBar scrollBar = getScrollbarComponent(tableView, orientation);
        if (scrollBar == null) {
            System.out.println("ScrollBar null");
            return;
        }
