...
public class DialogsApp extends Application {
    ...
    public void start(Stage primaryStage) {
        ...
        inputSingleLineButton.setOnAction(evt -> {
           Dialog<String> d = dialogPane.showTextInput("Text Input", "Please enter something, anything really.", "Text already there ...", false);
            System.out.println(d.valueProperty());
        });
        ...
        node1Button.setOnAction(evt -> {
            Dialog<Object> s = dialogPane.showNode(INFORMATION, "Select Person", createCustomNode(), List.of(new ButtonType("Send Mail", ButtonBar.ButtonData.OK_DONE), new ButtonType("Call", ButtonBar.ButtonData.APPLY)));

            System.out.println(s.getButton(s.getButtonTypes().get(0)));
            s.getButton(s.getButtonTypes().get(0)).setOnAction(e -> {
